...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.scrollArea.setGeometry(QtCore.QRect(50, 60, 700, 450))
        ...
        for i in range(10):
            card = self.createProductCard(
                f"Product {i+1}", "Category", f"{(i+1)*100}", f"{50 - i*2}"
            )
            row = i // 3
            col = i % 3
            self.gridLayout.addWidget(card, row, col)

    def createProductCard(self, name, category, price, stocks):
        ...
        # Category
        label_cat = QtWidgets.QLabel("category", parent=card)
        label_cat.setGeometry(QtCore.QRect(10, 210, 60, 16))
        input_cat = QtWidgets.QLineEdit(category, parent=card)
        input_cat.setGeometry(QtCore.QRect(10, 230, 180, 22))
